...
const getUSerAppointments = async()=>{
  ...
  const initPay = (order) => {
    const options = {
      ...
      amount: order.amount,     // required in some setups
      ...
      handler: async (response) => {
        ...
        try{
          ...
          if(data.success){
          await getUSerAppointments()   // <-- fetch updated list
        ...
        {appointments.map((item,index)=>(
          <div  className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {/* if cancelled is not true then only button shown */}
              ...
              {item.isCompleted && <button className='sm:min-w-48 py-2 border border-green-500 text0green-500'>Completed</button>  }
            </div>
          </div>
        ))}
      </div>
